 ...
 * @LastEditTime: 2019-05-17 17:41:44
 */
...
const jsxParse = require(resolveFlammae(
    './packages/flammae-utils/jsx-parse',
));
const markdownParse = require(resolveFlammae(
    './packages/flammae-utils/markdown-parse',
));
...
console.log('正在读取文件...\n');
parseFiles(
    getFilePaths(docDir, pattern).concat(getFilePaths(pageDir, pattern)),
);
...
function parseFiles(filePaths) {
    ...
    Promise.all(
        filePaths
            .map(filePath => new Promise((resolve) => {
                fs.readFile(
                    filePath, {
                        encoding: 'utf8',
                    },
    ...
    && direntArr.forEach((dirent) => {
        if (dirent.isFile()) {
            ...
            filesPaths = filesPaths.concat(
                getFilePaths(path.resolve(dir, dirent.name), callback),
            );
        }
    });
    return filesPaths;
